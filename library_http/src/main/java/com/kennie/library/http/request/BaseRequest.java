...
import static com.kennie.library.http.KennieHttp.getRetrofitBuilder;
import static com.kennie.library.http.KennieHttp.getRxCache;
...
import com.kennie.library.http.api.ApiService;
...
import com.kennie.library.http.interceptor.BaseDynamicInterceptor;
import com.kennie.library.http.interceptor.CacheInterceptor;
import com.kennie.library.http.interceptor.CacheInterceptorOffline;
import com.kennie.library.http.interceptor.HeadersInterceptor;
import com.kennie.library.http.interceptor.NoCacheInterceptor;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = KennieHttp.getContext();
        KennieHttp config = KennieHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) cacheTime = KennieHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = KennieHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = KennieHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0) KennieHttp.getCookieJar().addCookies(cookies);
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = KennieHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = KennieHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(KennieHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, KennieHttp.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(KennieHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(KennieHttp.getContext(), cacheControlValue);

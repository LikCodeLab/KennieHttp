package com.kennie.library.http;
...
import com.kennie.library.http.cache.converter.IDiskConverter;
import com.kennie.library.http.cache.converter.SerializableDiskConverter;
import com.kennie.library.http.cache.model.CacheMode;
import com.kennie.library.http.cookie.CookieManger;
import com.kennie.library.http.https.HttpsUtils;
import com.kennie.library.http.interceptor.HttpLoggingInterceptor;
import com.kennie.library.http.model.HttpHeaders;
import com.kennie.library.http.model.HttpParams;
import com.kennie.library.http.request.CustomRequest;
import com.kennie.library.http.request.DeleteRequest;
import com.kennie.library.http.request.DownloadRequest;
import com.kennie.library.http.request.GetRequest;
import com.kennie.library.http.request.PostRequest;
import com.kennie.library.http.request.PutRequest;
...
public final class KennieHttp {
    ...
    private volatile static KennieHttp singleton = null;
    ...
    public static KennieHttp getInstance() {
        testInitialize();
        if (singleton == null) {
            synchronized (KennieHttp.class) {
                if (singleton == null) {
                    singleton = new KennieHttp();
                }
            }
        }
        return singleton;
    ...
    private static void testInitialize() {
        if (sContext == null)
            throw new ExceptionInInitializerError("请先在全局Application中调用 KennieHttp.init() 初始化！");
    }
     ...
     * 调试模式,默认打开所有的异常调试
     */
    public KennieHttp debug(String tag) {
        debug(tag, true);
        return this;
    }
     ...
     * https的全局访问规则
     */
    public KennieHttp setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * https的全局自签名证书
     */
    public KennieHttp setCertificates(InputStream... certificates) {
     ...
     * https双向认证证书
     */
    public KennieHttp setCertificates(InputStream bksFile, String password, InputStream... certificates) {
     ...
     * 全局cookie存取规则
     */
    public KennieHttp setCookieStore(CookieManger cookieManager) {
     ...
     * 全局读取超时时间
     */
    public KennieHttp setReadTimeOut(long readTimeOut) {
     ...
     * 全局写入超时时间
     */
    public KennieHttp setWriteTimeOut(long writeTimeout) {
     ...
     * 全局连接超时时间
     */
    public KennieHttp setConnectTimeout(long connectTimeout) {
     ...
     * 超时重试次数
     */
    public KennieHttp setRetryCount(int retryCount) {
     ...
     * 超时重试延迟时间
     */
    public KennieHttp setRetryDelay(int retryDelay) {
     ...
     * 超时重试延迟叠加时间
     */
    public KennieHttp setRetryIncreaseDelay(int retryIncreaseDelay) {
     ...
     * 全局的缓存模式
     */
    public KennieHttp setCacheMode(CacheMode cacheMode) {
     ...
     * 全局的缓存过期时间
     */
    public KennieHttp setCacheTime(long cacheTime) {
     ...
     * 全局的缓存大小,默认50M
     */
    public KennieHttp setCacheMaxSize(long maxSize) {
     ...
     */
    public KennieHttp setCacheVersion(int cacheersion) {
        if (cacheersion < 0)
     ...
     */
    public KennieHttp setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public KennieHttp setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public KennieHttp setHttpCache(Cache cache) {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public KennieHttp addCommonParams(HttpParams commonParams) {
     ...
     * 添加全局公共请求参数
     */
    public KennieHttp addCommonHeaders(HttpHeaders commonHeaders) {
     ...
     * 添加全局拦截器
     */
    public KennieHttp addInterceptor(Interceptor interceptor) {
     ...
     * 添加全局网络拦截器
     */
    public KennieHttp addNetworkInterceptor(Interceptor interceptor) {
     ...
     * 全局设置代理
     */
    public KennieHttp setOkproxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public KennieHttp setOkconnectionPool(ConnectionPool connectionPool) {
     ...
     */
    public KennieHttp setOkclient(OkHttpClient client) {
     ...
     */
    public KennieHttp addConverterFactory(Converter.Factory factory) {
     ...
     */
    public KennieHttp addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public KennieHttp setCallbackExecutor(Executor executor) {
     ...
     */
    public KennieHttp setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * 全局设置baseurl
     */
    public KennieHttp setBaseUrl(String baseUrl) {
